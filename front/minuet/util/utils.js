...
export const generateCircles = (
  ...
  const sum = sortedKeywords.reduce(
    (acc, current) => acc + current.frequency,
    0
  );
...
export const drawCirclesOnCanvas = (
  circles,
  canvas,
  drawContainerCircle,
  ...
  circles.forEach((circle) => {
    ...
    ctx.lineWidth = 5; // Adjust the border width as needed
    ...
    ctx.font = `bold ${Math.floor(circle.size / 4)}px sans-serif`;
    ctx.fillStyle = "black";
