          ...
          <Animated.View style={[rStyle, { marginVertical: 10 }]}>
              ...
              <View style={styles.iconContainer}>
                ...
                >
                  {isLike ? (
                    ...
                    <Ionicons name="heart-circle" size={40} color="black" />
                  )}
                </TouchableOpacity>
                <TouchableOpacity
                  activeOpacity={0.7}
                ...
                {/* <Feather name="message-circle" size={24} style={styles.icon} color="white" />
              <FontAwesome name="paper-plane-o" size={24} style={styles.icon} color="white" /> */}
              </View>
              <View style={styles.iconContainer}>
            ...
            <View style={styles.summaryContainer}>
              <Text style={{ color: "black" }}>
                {loadMore && item.summary.length > 50
                  ? item.summary.slice(0, 50) + "..."
                  : item.summary}
                {!loadMore && item.summary}
              </Text>
              {loadMore && (
                <TouchableOpacity onPress={() => setLoadMore(!loadMore)}>
                  <Text style={{color:"black"}}>[더보기]</Text>
                </TouchableOpacity>
              )}
            </View>
            </View>
                ...
                >
                  {searchResult.map((item, index) => (
                    <Animated.View key={index}>
                      <Text>{removeHTMLTags(item.description)}</Text>
                      <View style={{ flexDirection: "row", marginBottom: 10 }}>
                        <Text>[링크] : </Text>
                        <TouchableOpacity onPress={() => handlePress(item.link)}>
                          <Text
                            style={{color:GlobalColor.colors.primary_blue}}
                          >
                            이동하기
                          </Text>
                        </TouchableOpacity>
